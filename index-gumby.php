<!doctype html>
<html>
<head>
	<meta charset="utf-8" />
	<title>TIFN - Triad Indie Film Network</title>

	<link rel="stylesheet" href="css/style.css">

	<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js"></script>
	<script src="/js/scrollspy.js"></script>

	<link href='http://fonts.googleapis.com/css?family=Ribeye+Marrow' rel='stylesheet' type='text/css'>

</head>


<body data-spy="scroll">

<div class="christmas-lights">
</div>

<header>
	<div class="container">
		<div class="row">
			<div class="twelve columns">
				<a href="http://www.facebook.com/TriadIndie" title="TIFN on Facebook">
					<div id="facebook" class="one columns alpha"></div>
				</a>
				<div id="logo" class="ten columns">
					<h1><a href="#fruitcake-img" title="TRIAD INDIE FILM NETWORK">TRIAD INDIE FILM NETWORK</a></h1>
				</div>
				<a href="http://twitter.com/TriadIndieFilm" title="TIFN on Twitter">
					<div id="twitter" class="one columns omega"></div>
				</a>
			</div>
		</div>
	</div>

	<div class="container">
		<div class="row">
			<nav id="navbar" class="twelve columns">
				<ul>
					<!-- <li class="active"><a href="#welcome" title="Welcome">Welcome</a></li> -->
					<li><a href="#welcome" title="Welcome">Welcome</a></li>
					<li><a href="#about" title="About">About</a></li>
					<li><a href="#membership" title="Membership">Membership</a></li>
					<li><a href="#events" title="Events">Events</a></li>
					<li><a href="#links" title="Links">Links</a></li>
					<li><a href="#contact" title="Contact">Contact</a></li>
				</ul>
			</nav>
		</div>
	</div>
</header>

<div class="container">
	<div class="row">
		<div class="twelve columns">
			<div id="fruitcake-img">
				<div class="content-container">
				<a href="fruitcake" title="Fruit Cake Film Challenge Film Festival Challenge">
						<img src="img/fruit-cake.jpg" alt="Fruit Cake">
					<p class="film-challenge">The 2012 Fruit Cake Film Challenge Film Festival</p>
					<p class="click-here">Click to Enter</p>
					</a>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="container">
	<div class="row">
		<div id="welcome" class="twelve columns">
			<div class="content-container">
				<a class="back-to-fruitcake" style="float:left;" href="fruitcake" title="Back to Fruitcake"><img src="img/fruitcake-icon.png" alt="Fruitcake Film Challenge Film Festival">Back to the Fruitcake</a>
				<h2>Welcome to TIFN</h2>
				<div class="grid_6 alpha">
					<p>The Triad Indie Film Network (TIFN) is an organization of anyone and everyone that is interested in creating and/or enjoying locally produced short and feature-length films, whether they be narrative fiction, documentary, animation, or some clever combination thereof. TIFN is based out of the Piedmont Triad area of North Carolina, which encompasses Greensboro, High Point, Winston-Salem and the surrounding communities. We have a brand new home at <a href="http://www.greensboroschoolofcreativity.org/" target="blank" title="The Creative Center">The Creative Center</a>, along with a committee of officers, and a renewed sense to promote the cinematic efforts of Triad filmmakers.</p>
				</div>
				<div class="grid_5 omega prefix_1">
					<p>Membership is open to aspiring and experienced writers, directors, producers, actors, crew, musicians and audiences of all ages and backgrounds.</p>
					<a class="btn" href="#membership" title="Click to Join TIFN"> Click to Join TIFN » </a>
				</div>
			</div>
		</div>
	</div>
</div>


<div id="about" class="twelve columns">
	<div class="content-container">
		<a class="top-please" style="float:right;" href="#welcome" title="Top Please">Top Please</a>
		<a class="back-to-fruitcake" style="float:left;" href="fruitcake" title="Back to Fruitcake"><img src="img/fruitcake-icon.png" alt="Fruitcake Film Challenge Film Festival">Back to the Fruitcake</a>
		<h2>About TIFN</h2>
			<p>Triad Indie Film Network strives to create a supportive community of filmmakers & film-lovers. We have regular screenings each month at the TIFN Headquarters, in our brand new HiDef screening room that comes equipped with a <em>120" screen</em> and <em>5.1 Surround Sound</em> system. We also have a 1000-square-foot studio space and shooting stage, all located mere minutes from downtown Greensboro, and all readily available for TIFN members and supporters. We offer a variety of directing, acting & screenwriting groups, as well other types of networking meetings.</p>

		<div class="six columns alpha">
			<img src="img/screen-room.jpg" alt="Film Screening Room">
		</div>
		<div class="six columns omega">
			<img src="img/sound-stage.jpg" alt="Sound Stage">
		</div>
	</div>
</div>

<div id="membership" class="twelve columns">
	<div class="content-container">
		<a class="top-please" style="float:right;" href="#welcome" title="Top Please">Top Please</a>
		<a class="back-to-fruitcake" style="float:left;" href="fruitcake" title="Back to Fruitcake"><img src="img/fruitcake-icon.png" alt="Fruitcake Film Challenge Film Festival">Back to the Fruitcake</a>
		<h2>Membership</h2>
			<div class="six columns alpha">
				<p>A TIFN Membership is open to both aspiring and experienced writers, directors, producers, actors, crew, musicians and film lovers of all ages and backgrounds. The membership term is yearly, beginning on January 1st of the calendar year and is pro-rated if joined after January.</p>
				<br>
				<h3>TIFN Membership Rates for 2012</h3>
				<table>
					<tr>
						<td><strong>$25</strong></td>
						<td>Individuals</td>
					</tr>
					<tr>
						<td><strong>$12.50</strong></td>
						<td>Students/Seniors** with valid id</td>
					</tr>
					<tr>
						<td><strong>$62.50</strong></td>
						<td>Group 2-5 members</td>
					</tr>
					<tr>
						<td><strong>$125</strong></td>
						<td>Group 6-10 members</td>
					</tr>
				</table>
			</div>
			<div class="six columns prefix_1 omega">
				<h3>Application</h3>
				<p><em>Please submit your application on line.</em></p>
			</div>
			<div class="three columns prefix_1">
				<a class="btn" href="/application/online-app.php" title="Submit Application Online" style="margin:0px 0px 30px 0px;"> Submit Online» </a>
			</div>
			<div class="six columns prefix_1 omega">
				<p><em>You may also download and print via the icons below.</em></p>
			</div>
			<div class="two columns prefix_1">
				<a href="application/tifn-membership-application.doc" title="TIFN Membership Application Doc">
					<div id="word-icon"></div>
				</a>
			</div>
			<div class="two columns">
					<a href="application/tifn-membership-application.pdf" title="TIFN Membership Application PDF">
						<div id="pdf-icon"></div>
					</a>
			</div>
	</div>
</div>

<div id="events" class="twelve columns">
	<div class="content-container">
		<a class="top-please" style="float:right;" href="#welcome" title="Top Please">Top Please</a>
		<a class="back-to-fruitcake" style="float:left;" href="fruitcake" title="Back to Fruitcake"><img src="img/fruitcake-icon.png" alt="Fruitcake Film Challenge Film Festival">Back to the Fruitcake</a>
		<h2>Events</h2>
		<div class="twelve columns alpha">
			<p>Join us on the first Tuesday of every month, in the Film Room (Rm. 213) of <a href="http://www.greensboroschoolofcreativity.org/" target="blank" title="The Creative Center">The Creative Center</a>, for the monthly TIFN meetings. The meetings will consist of official TIFN business, networking and miscellaneous movie matters (short film screenings, speakers, presentations, etc.).</p>
			<br>
		</div>
<div class="six columns alpha">
			<h3>Next Official TIFN Meeting</h3>
			<table>
				<tr>
					<td><strong>When:</strong></td>
					<td>Tuesday, November, 6th @ 7pm - ???</td>
				</tr>
				<tr>
					<td><strong>Where:</strong></td>
					<td>The Creative Center - <a href="http://g.co/maps/kkcx5" target="blank" title="View Map">View Map</a><br></td>
				</tr>
			</table>
			<h3>Special Events</h3>
			<p>No special events are scheduled at this time, except for being awesome, which we make sure to scheduled everyday.</p>
		</div>
		<div class="six columns omega prefix_1">
			<h3>This Months Meeting</h3>
			<p>Join us as we elect our officers for 2013. Do you want to serve as an officer? Do you want to nominate someone? Come to the meeting and we'll take nominations and vote in the new officers.</p>
			<a class="btn" href="bylaws.php" title="From the Bylaws" style="margin:0px 0px 30px 0px;"> From the bylaws » </a>
		</div>
	</div>
</div>

<div id="links" class="twelve columns">
	<div class="content-container">
		<a class="top-please" style="float:right;" href="#welcome" title="Top Please">Top Please</a>
		<a class="back-to-fruitcake" style="float:left;" href="fruitcake" title="Back to Fruitcake"><img src="img/fruitcake-icon.png" alt="Fruitcake Film Challenge Film Festival">Back to the Fruitcake</a>
		<h2>Links & Resources</h2>
		<div class="seven columns alpha">
			<h3>Friends of TIFN</h3>
			<table>
				<tr>
					<td><strong>Jason Pierce:</strong></td>
					<td><a href="http://filmwhisperer.com/" target="blank">Film Whisperer</a></td>
					<td></td>
				</tr>
				<tr>
					<td><strong>Les Butchart:</strong></td>
					<td><a href="http://newriverreleasing.com/" target="blank">New River Releasing</a></td>
					<td><a href="http://fugitivepoetspress.com/" target="blank">Fugitive Poets Press</a></td>
				</tr>
				<tr>
					<td><strong>Seth Hall:</strong></td>
					<td><a href="http://newriverreleasing.com/" target="blank">New River Releasing</a></td>
					<td><a href="http://middle8media.com.com/" target="blank">Middle8Media</a></td>
				</tr>
				<tr>
					<td><strong>The Creative Center:</strong></td>
					<td><a href="http://www.thecreativecenter.net//" target="blank">The Creative Center</a></td>
				</tr>
			</table>
		</div>
		<div class="five columns omega prefix_1">
			<h3>Filmmaking Resources</h3>
				<ul>
					<li><a href="http://moviesamongfriends.com/" target="_blank" title="Movies Among Friends">Movies Among Friends</a></li>
					<li><a href="http://fracturedatlas.org/" target="_blank" title="Fractured Atlas">Fractured Atlas</a></li>
					<li><a href="http://filmspecific.com/" target="_blank" title="Film Specific">Film Specific</a></li>
					<li><a href="http://nofilmschool.com/" target="_blank" title="No Film School">No Film School</a></li>
					<li><a href="http://scripped.com/" target="_blank" title="Scripped">Scripped</a></li>
					<li><a href="https://www.scenechronize.com/" target="_blank" title="Scenecronize">Scenecronize</a></li>
		</div>
	</div>
</div>


<div id="contact" class="twelve columns">
	<div class="content-container">
		<a class="top-please" style="float:right;" href="#welcome" title="Top Please">Top Please</a>
		<a class="back-to-fruitcake" style="float:left;" href="fruitcake" title="Back to Fruitcake"><img src="img/fruitcake-icon.png" alt="Fruitcake Film Challenge Film Festival">Back to the Fruitcake</a>
		<h2>Contact</h2>
			<div class="six columns alpha">
			<h3>Address</h3>
					<address>
				        <strong>Triad Indie Film Network</strong><br>
				        c/o The Creative Center<br>
				        900 Sixteenth Street<br>
				        Greensboro, NC 27405<br>
				        <a href="http://g.co/maps/kkcx5" target="blank" title="View Map">View Map</a><br>
	    			</address>
					<h3>Email</h3>
					<table>
						<tr>
							<td><strong>General Inquiries:</strong></td>
							<td><a href="mailto:tifn@triadindie.org">TIFN</a></td>
						</tr>
						<tr>
							<td><strong>President:</strong></td>
							<td><a href="mailto:president@triadindie.org">Jason Pierce</a></td>
						</tr>
						<tr>
							<td><strong>Vice President:</strong></td>
							<td><a href="mailto:vice-president@triadindie.org">Les Butchart</a></td>
						</tr>
						<tr>
							<td><strong>Secretary:</strong></td>
							<td><a href="mailto:secretary@triadindie.org">Louis Di Eugenio</a></td>
						</tr>
						<tr>
							<td><strong>Treasurer:</strong></td>
							<td><a href="mailto:treasurer@triadindie.org">Jon Clowdis</a></td>
						</tr>
						<tr>
							<td><strong>Rental Coordinator:</strong></td>
							<td><a href="mailto:equipment@triadindie.org">Mark Wagoner</a></td>
						</tr>
						<tr>
							<td><strong>Marketing Coordinator:</strong></td>
							<td><a href="mailto:marketing@triadindie.org">Bernie Bohigas</a></td>
						</tr>
						<tr>
							<td><strong>Web Master:</strong></td>
							<td><a href="mailto:webmaster@triadindie.org">Seth Hall</a></td>
						</tr>

					</table>
			</div>

			<div class="six columns omega prefix_1">
			<h3>Map</h3>
	    		<iframe width="380" height="380" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="http://maps.google.com/maps?q=900+Sixteenth+Street+Greensboro,+NC+27405&amp;client=safari&amp;oe=UTF-8&amp;hnear=900+16th+St,+Greensboro,+North+Carolina+27405&amp;gl=us&amp;t=h&amp;ie=UTF8&amp;hq=&amp;ll=36.11569,-79.777822&amp;spn=0.024268,0.036478&amp;z=14&amp;iwloc=A&amp;output=embed"></iframe>
<!-- 				<form name="contactform" method="post" action="send_form_email.php">
					<table>
					<tr>
					 <td>
					  <input  area-required="true" required type="text" name="name" placeholder="name" maxlength="50" size="30">
					 </td>
					</tr>
					<tr>
					 <td>
					  <input  area-required="true" required type="email" name="email" placeholder="email" maxlength="80" size="30">
					 </td>
					</tr>
					<tr>
					 <td>
					  <textarea  area-required="true" required name="message" placeholder="message" maxlength="1000" cols="25" rows="6" style="width:98%;"></textarea>
					 </td>
					</tr>
					<tr>
					 <td class="footer-contact-button" colspan="2" style="text-align:center; float:right;">
					  <input type="submit" class="btn" value="Submit »">
					</td>
					</tr>
					</table>
				</form> -->
			</div>
	</div>
</div>

<div class="twelve columns">
	<footer>
		<span>Copyright © 2012 TIFN</span>
		<span>brought to you by <a href="http://middle8media.com" target="_blank" title="M8M Web Design">Middle8Media</a></span>
	</footer>
</div>

</div>

<script type='text/javascript' src='js/app-ck.js'></script>

</body>
</html>